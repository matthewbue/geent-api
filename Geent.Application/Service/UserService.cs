...
namespace Geent.Application.Service
{
    public class UserService : IUserService
    {
        ...
        public async Task SaveProfileImageAsync(int userId, byte[] profileImage)
        {
            if (profileImage == null || profileImage.Length == 0)
            {
                throw new ArgumentException("A imagem de perfil não pode ser nula ou vazia.");
            }

            await _userRepository.SaveProfileImageAsync(userId, profileImage);
        }

        public async Task<byte[]> GetProfileImageAsync(int userId)
        {
            var profileImage = await _userRepository.GetProfileImageAsync(userId);
            if (profileImage == null)
            {
                throw new Exception("Imagem de perfil não encontrada.");
            }
